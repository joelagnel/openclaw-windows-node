...
public sealed class AppRefactorContractTests
{
    ...
    public void CapabilitiesPage_FiltersModelsBySelectedGpuCapacityAndShowsMemoryEvidence()
    {
        ...
        Assert.Contains("{FormatMemorySize(eligibility.RequiredTotalMemoryBytes)} required", source);
        Assert.Contains("{FormatOptionalMemorySize(eligibility.DetectedTotalMemoryBytes)} CUDA-visible on {gpu.Name}", source);

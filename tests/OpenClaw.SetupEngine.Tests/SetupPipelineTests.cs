...
public class SetupPipelineTests
{
    ...
    public void BuildDefaultSteps_IncludesCurrentSetupFlow()
    {
        ...
        Assert.True(steps.FindIndex(s => s is VerifyLocalAiWslStep) < configureGatewayIndex);
        Assert.True(steps.FindIndex(s => s is ConfigureLocalAiGatewayStep) < installServiceIndex);

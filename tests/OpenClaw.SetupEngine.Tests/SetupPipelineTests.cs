...
public class SetupPipelineTests
{
    ...
    public void BuildDefaultSteps_IncludesCurrentSetupFlow()
    {
        ...
        Assert.IsType<AcquireLocalAiRuntimeStep>(steps[5]);
        Assert.IsType<AcquireLocalAiModelStep>(steps[6]);
        Assert.IsType<PersistLocalAiManifestStep>(steps[7]);
        Assert.IsType<StartLocalAiRuntimeStep>(steps[8]);
        Assert.IsType<CaptureLocalAiGpuBaselineStep>(steps[9]);
        Assert.IsType<VerifyLocalAiInferenceStep>(steps[10]);
        Assert.IsType<VerifyLocalAiGpuLoadStep>(steps[11]);
        Assert.IsType<EnsureWslPlatformStep>(steps[12]);
